    ...
    <div class="form card col-span-2 xl:col-span-1">
        <div class="card-header capitalize font-bold text-xl">Edit Kamar</div>
            <div class="w-full text-left px-6 py-10">
                <form action="{{ route('simpan-fasilitas-umum') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('POST')
                    <div class="fasilitas_umum">
                        <label for="fasilitas_umum" class="block mb-2 font-semibold">Fasilitas Umum</label>
                        <input type="text" name="fasilitas_umum" id="fasilitas_umum" class="border-b-2 border-solid w-55 focus:outline-none p-2 rounded-sm" placeholder="Masukan fasilitas umum">
                    </div>
                    <div class="keterangan mt-5">
                        <label for="keterangan" class="block mb-2 font-semibold">Keterangan</label>
                        <input type="text" name="keterangan" id="keterangan"  class="border-b-2 border-solid w-55 focus:outline-none p-2 rounded-sm" placeholder="Masukan jumlah kamar">
                    </div>
                    <div class="gambar_fasilitas mt-5">
                        <label for="gambar_fasilitas" class="block mb-2 font-semibold">Gambar Fasilitas</label>
                        <input type="file" name="gambar_fasilitas" id="gambar_fasilitas" class="border-b-2 border-solid w-55 focus:outline-none p-2 rounded-sm">
                    </div>
